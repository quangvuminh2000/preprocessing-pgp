...
COMMON_EMAIL_REGEX = (
    "^[a-z0-9][a-z0-9_\.]{4,31}[a-z0-9]@[a-z0-9]{2,}(?:\.[a-z0-9]{2,12}){1,2}$"
)
...
def check_valid_email(f_emails: pd.DataFrame, email_col: str = "email") -> pd.DataFrame:
    ...
    repls = {"nan": np.nan, "none": np.nan, "": np.nan, None: np.nan}
    f_emails[email_col + "_convert"] = (
        f_emails[email_col]
        .str.lower()
        .str.strip()
        .str.replace("\s+", "", regex=True)
    ...
    accented = f_emails["email"].str.encode("ascii", errors="ignore") != f_emails[
        "email"
    ...
    for email_service in EMAIL_CONFIGS.values():
        ...
        f_emails.loc[
            ...
            "is_email_valid",
        ] = f_emails[email_col].str.contains(
            email_service["regex"], regex=True, na=False
        )

    # Common email: not in large email companies.
    f_emails.loc[
        (f_emails["is_email_valid"] & ~f_emails["email_group"].isin(CHECK_DOMAINS)),
        "is_email_valid",
    ...
    f_emails.loc[
        f_emails[email_col].str.contains(
            "@privaterelay.appleid.com|[0-9]+\_autoemail", regex=True, na=False
        ),
        "is_email_valid",
    ] = False
    return f_emails
...
def _validate_phone_email(
    df_profile: pd.DataFrame, email_col: str = "email", phone_col: str = "phone"
) -> pd.DataFrame:
    df_profile = df_profile.pipe(check_valid_phone, phone_col=phone_col).pipe(
        check_valid_email, email_col=email_col
    )
    df_profile.drop(columns=["email_name", "email_group", "email_len"], inplace=True)
    df_profile.loc[
        (df_profile["is_phone_valid"] == True) & (df_profile["phone_convert"].isna()),
        "phone_convert",
...
def validate_phone_email(
    ...
    if n_cores > 1:
        ...
        try:
            ...
            df_profile_val = pd.concat(
                pool.map(
                    partial(
                        _validate_phone_email, email_col=email_col, phone_col=phone_col
                    ),
                    df_splits,
                )
            )
...
def MetaDataPhone(valid_phone):
    # LOAD DICTIONARY
    subphone_vn = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/sa/cdp/data/vn_sub_phone.parquet",
        filesystem=hdfs,
        columns=["NewSubPhone", "PhoneVendor"],
    )
    ...
    valid_phone.loc[
        valid_phone["phone"].str[:3].isin(df_subtele_vn["sub_phone"].unique()),
        "phone_type",
    ] = "landline"
    valid_phone.loc[
        valid_phone["phone"].str[:3].isin(subphone_vn["sub_phone"].unique())
        & valid_phone["phone_type"].isna(),
        "phone_type",
    ] = "mobile phone"
    ...
    valid_phone_landline["sub_phone"] = (
        valid_phone_landline["phone"].str[:4].astype(str)
    )
    valid_phone_landline = valid_phone_landline.merge(
        df_subtele_vn, how="left", on=["sub_phone"]
    )
    valid_phone_landline.loc[
        (valid_phone_landline["phone"].str[:3] == "024")
        & valid_phone_landline["phone_vendor"].isna(),
        "phone_vendor",
    ] = "Hà Nội"
    valid_phone_landline.loc[
        (valid_phone_landline["phone"].str[:3] == "028")
        & valid_phone_landline["phone_vendor"].isna(),
        "phone_vendor",
    ] = "Thành phố Hồ Chí Minh"

    valid_phone_mobile["sub_phone"] = valid_phone_mobile["phone"].str[:3]
    valid_phone_mobile = valid_phone_mobile.merge(
        subphone_vn, how="left", on=["sub_phone"]
    )

    valid_phone = pd.concat(
        [valid_phone_landline, valid_phone_mobile, valid_phone_none], ignore_index=True
    )
    ...
    for name_case, format_case in dict_beauty_phone.items():
        ...
        # assign
        valid_phone.loc[
            valid_phone["tail_phone_type"].isna()
            & valid_phone["phone"].str[-size_case:].isin(format_case),
            "tail_phone_type",
        ] = name_case

    valid_phone.loc[
        valid_phone["tail_phone_type"].isna() & valid_phone["phone"].notna(),
        "tail_phone_type",
    ] = "Số Thường"
...
def FormatName(valid_email):
    ...
    # customer_type
    name_email = (
        valid_email[valid_email["username"].notna()][["username"]]
        .copy()
        .drop_duplicates()
    )
    ...
    # process format name
    com_name_email = name_email[
        name_email["customer_type"].notna()
        | (name_email["name"].str.split(" ").str.len() > 3)
    ].copy()
    com_name_email["format_name"] = com_name_email["name"]

    per_name_email = name_email[
        ~name_email["name"].isin(com_name_email["name"].unique())
    ].copy()[["name"]]
    dict_trash = {
        "": None,
        "Nan": None,
    ...
    with mp.Pool(8) as pool:
        per_name_email[["last_name", "middle_name", "first_name"]] = pool.map(
            preprocess_lib.SplitName, per_name_email["name"]
        )
    columns = ["last_name", "middle_name", "first_name"]
    per_name_email["format_name"] = (
        per_name_email[columns]
        .fillna("")
        .agg(" ".join, axis=1)
    ...
    per_name_email["format_name"] = (
        per_name_email["format_name"].str.strip().replace(dict_trash)
    )
    per_name_email = per_name_email.drop(
        columns=["last_name", "middle_name", "first_name"]
    )
    ...
    # fillna 'Ca nhan'
    valid_email.loc[
        valid_email["username"].notna() & valid_email["customer_type"].isna(),
        "customer_type",
    ] = "Ca nhan"

    # Fill Accent + gender
    map_name = (
        valid_email[valid_email["customer_type"] == "Ca nhan"][["username"]]
        .drop_duplicates()
        .copy()
    )
    ...
    valid_email.loc[valid_email["predict_name"].isna(), "predict_name"] = valid_email[
        "username"
    ]
    valid_email["username"] = valid_email["predict_name"]

    valid_email.loc[
        valid_email["predict_gender"].isna(), "predict_gender"
    ] = valid_email["gender"]
...
def ValidPhoneEmail(date_str):
    ...
    def load_email(cttv, key):
        emails = pd.read_parquet(
            f"{raw_path}/{cttv}.parquet/d={today}",
            filesystem=hdfs,
            columns=[key, "email"],
        )
    ...
    #     for f in FPTS:
    #         f_profile = pd.read_parquet(
    #             f'{raw_path}/{f}.parquet/d={today}',
    #             columns=['id', 'email'],
    #             filesystem=hdfs
    #         )
    #         f_profile['id'] = f + '-' + f_profile['id'].astype(str)
    #         emails_bank = emails_bank.append(f_profile, ignore_index=True)

    # Regex syntax
    emails_bank["email"] = (
        emails_bank["email"].str.strip().str.replace("\s+", "", regex=True)
    )
    ...
    # Mapping email
    raw_mapping_emails = (
        raw_f_emails[raw_f_emails["email"].notna()][["email", "email_convert"]]
        .copy()
    ...
    # Pass syntax
    f_emails = (
        raw_f_emails[raw_f_emails["is_email_valid"]][["id", "email"]]
        .copy()
        .drop_duplicates()
    )

    raw_stats_email = (
        f_emails.groupby(by=["email"])["id"].agg(count_id="count").reset_index()
    )
    ...
    profile_fo = pd.read_parquet(
        "/data/fpt/fdp/fo/dwh/user_profile.parquet/",
        filesystem=hdfs,
        columns=[
            "vne_id",
            "name",
    ...
    email_fo.loc[
        email_fo["name"].str.contains("^[0-9]*$", case=False, na=False), "name"
    ] = None
    ...
    temp_profile_fo = profile_fo[
        profile_fo["email"].isin(stats_email["email"])
        & (profile_fo["name"].str.strip() == "")
        & (profile_fo["cookie"].str.strip() != "")
    ].copy()

    ### vừa tạo + không dùng
    temp_profile_fo = temp_profile_fo[
        temp_profile_fo["created_date"].dt.date
        == temp_profile_fo["last_visited"].dt.date
    ]
    stats_temp_profile_fo = (
        temp_profile_fo.groupby(by=["cookie"])["vne_id"]
        .agg(count_vne_id="count")
        .reset_index()
    )

    ### cùng lúc, 1 cookie nhiều account
    cookies_chaos = stats_temp_profile_fo[stats_temp_profile_fo["count_vne_id"] > 1][
        "cookie"
    ].tolist()
    email_chaos_fo = temp_profile_fo[temp_profile_fo["cookie"].isin(cookies_chaos)][
        "email"
    ].tolist()

    ### email chỉ xài ở 1 CTTV thì có khả năng là email rác nhiều hơn.
    email_warning_fo["email"] = stats_email[
        stats_email["email"].isin(email_chaos_fo) & (stats_email["count_id"] == 1)
    ]["email"].tolist()
    ...
    profile_fplay = pd.read_parquet(
        f"{raw_path}/fplay.parquet/d={today}",
        filesystem=hdfs,
        columns=["user_id_fplay", "phone", "email", "name"],
        filters=[("email", "in", stats_email["email"].unique())],
    ).rename(columns={"user_id_fplay": "id"})

    email_fplay = profile_fplay[
        profile_fplay["phone"].notna() & profile_fplay["email"].notna()
    ].copy()
    ...
    email_fplay.loc[
        email_fplay["name"].str.contains("^[0-9]*$", case=False, na=False), "name"
    ] = None

    email_fplay["username_email"] = email_fplay["email"].str.split("@").str[0]
    email_fplay.loc[
        email_fplay["username_email"].str.contains("^[0-9]*$", case=False, na=False),
        "username_email",
    ] = None
    email_fplay = email_fplay[email_fplay["username_email"].notna()].drop_duplicates()
    email_fplay.loc[email_fplay["name"].isna(), "name"] = email_fplay.loc[
        email_fplay["name"].isna(), "username_email"
    ]
    ...
    email_ok_fplay = email_ok_fplay[
        email_ok_fplay["email"].isin(stats_email["email"])
    ...
    email_ok_fplay["email"] = profile_fplay[profile_fplay["phone"].notna()][
        "email"
    ].tolist()
    ...
    ### user chỉnh lại tên
    word_name = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/word_name.parquet",
        filesystem=hdfs,
    )["word_name"].tolist()
    ...
    temp_profile_fplay = profile_fplay[
        profile_fplay["email"].isin(stats_email["email"])
    ].copy()
    temp_profile_fplay = temp_profile_fplay[
        temp_profile_fplay["name"].str.split(" ").str.len() > 1
    ]
    temp_profile_fplay["name_valid"] = temp_profile_fplay["name"].apply(CheckName)

    email_ok_fplay = pd.DataFrame()
    email_ok_fplay["email"] = temp_profile_fplay[
        temp_profile_fplay["name_valid"] == True
    ]["email"].tolist()
    ...
    stats_domain_email = (
        raw_stats_email.groupby(by=["domain_email"])["email"]
        .agg(count_email="count")
        .reset_index()
    )
    bugs_domain_email = stats_domain_email[stats_domain_email["count_email"] == 1][
        "domain_email"
    ].tolist()
    ...
    email_warning_domain["email"] = stats_email[
        stats_email["domain_email"].isin(bugs_domain_email)
    ]["email"].tolist()
    email_warning_domain["description"] = "Warning - Domain email"
    email_warning_domain = email_warning_domain[
        ~email_warning_domain["email"].isin(email_warning_cdp["email"])
    ]

    email_warning_cdp = email_warning_cdp.append(
        email_warning_domain, ignore_index=True
    )
    stats_email = stats_email[
        ~stats_email["email"].isin(email_warning_cdp["email"])
    ].copy()

    # Check dict email (extract name)
    email_dict = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/sa/cdp/data/email_dict_valid_latest.parquet",
        filesystem=hdfs,
        filters=[("email", "in", stats_email["email"].unique())],
    )
    ...
    temp_ok_email_extract = temp_ok_email_extract[
        ~temp_ok_email_extract["email"].isin(email_ok_cdp["email"])
    ]

    email_ok_cdp = email_ok_cdp.append(temp_ok_email_extract, ignore_index=True)
    stats_email = stats_email[~stats_email["email"].isin(email_ok_cdp["email"])].copy()

    # # Use lin vistickedword
    # def CheckSplitWordsEn(username):
    #     os.system('cls')
    #     words = wordninja.split(username)
    #     for word in words:
    #         if len(word) >= 4:
    #             return True
    #     return False

    # def StringSplitWordsVi(username):
    #     os.system('cls')
    #     words = split_words(username)
    #     text = ''
    #     for word in words:
    ...
    # #  %%capture
    # stats_email['user_email'] = stats_email['email'].str.split('@').str[0]
    # stats_email['user_email'] = stats_email['user_email'].str.replace(r'[^a-z]', '', regex=True).str.strip()

    # stats_email['check_split_en'] = stats_email['user_email'].apply(CheckSplitWordsEn)

    # stats_email['string_split_vi'] = stats_email['user_email'].apply(StringSplitWordsVi)
    # stats_email['string_split_vi'] = stats_email['string_split_vi'].str.strip()

    # stats_email['percent_split_vi'] = stats_email['string_split_vi'].str.len()/stats_email['user_email'].str.len()
    # stats_email['percent_split_vi'] = stats_email['percent_split_vi'].fillna(0)

    # email_warning_username_list = stats_email[~((stats_email['check_split_en'] == True) |
    #                                             (stats_email['percent_split_vi'] >= 0.4) &
    #                                             (stats_email['user_email'].str.len() > 2))]['email'].tolist()

    # ## rule base warning email
    # email_warning_username = pd.DataFrame()
    # email_warning_username['email'] = stats_email[stats_email['email'].isin(email_warning_username_list) &
    #                                               (stats_email['count_id'] == 1)]['email'].tolist()
    # email_warning_username['description'] = 'Warning - Username email'
    # email_warning_username = email_warning_username[~email_warning_username['email'].isin(email_warning_cdp['email'])]

    # email_warning_cdp = email_warning_cdp.append(email_warning_username, ignore_index=True)
    # stats_email = stats_email[~stats_email['email'].isin(email_warning_cdp['email'])].copy()

    # ## rule base ok email
    # email_ok_username = pd.DataFrame()
    # email_ok_username['email'] = stats_email['email']
    # email_ok_username['description'] = 'Ok - Username email'

    # email_ok_username = email_ok_username[~email_ok_username['email'].isin(email_ok_cdp['email'])]

    # email_ok_cdp = email_ok_cdp.append(email_ok_username, ignore_index=True)

    # Backup
    email_ok_backup = pd.DataFrame()
    email_ok_backup["email"] = list(stats_email["email"].unique())
    email_ok_backup["description"] = "Ok - Backup"
    email_ok_backup = email_ok_backup[
        ~email_ok_backup["email"].isin(email_ok_cdp["email"])
    ]
    ...
    final_stats_email = final_stats_email.drop_duplicates(
        subset=["email"], keep="first"
    )
    final_stats_email = raw_stats_email[["email", "count_id"]].merge(
        final_stats_email, how="left", on=["email"]
    )
    ...
    check_emails.loc[
        check_emails["description"].isna(), "description"
    ] = "Warning - Syntax email"
    ...
    #             filesystem=hdfs
    #         )
    #         phones_bank = phones_bank.append(f_profile, ignore_index=True)

    phones_bank = (
        phones_bank[phones_bank["phone"].notna()][["phone"]].drop_duplicates().copy()
    )
    check_phones = check_valid_phone(phones_bank, "phone")
    check_phones.columns = ["phone_raw", "is_phone_valid", "phone"]

    #     # UPDATE (d={date})
    #     yesterday_str = (datetime.strptime(today, '%Y-%m-%d') - timedelta(days=1)).strftime('%Y-%m-%d')
    #     yesterday_check_emails = pd.read_parquet(f'{utils_path}/valid_email.parquet/d={yesterday_str}', filesystem=hdfs)
    #     yesterday_check_phones = pd.read_parquet(f'{utils_path}/valid_phone.parquet/d={yesterday_str}', filesystem=hdfs)

    #     new_check_emails = check_emails[~check_emails['email_raw'].isin(yesterday_check_emails['email_raw'].unique())]
    #     new_check_phones = check_phones[~check_phones['phone_raw'].isin(yesterday_check_phones['phone_raw'].unique())]

    #     new_check_phones = MetaDataPhone(new_check_phones)

    #     final_check_emails = pd.concat([yesterday_check_emails, new_check_emails], ignore_index=True)
    #     final_check_phones = pd.concat([yesterday_check_phones, new_check_phones], ignore_index=True)

    #     # SAVE (d={date})
    #     final_check_emails['d'] = date_str
    #     final_check_emails.to_parquet(f'{utils_path}/valid_email.parquet',
    #                                   filesystem=hdfs,
    #                                   index=False,
    #                                   partition_cols='d')

    #     final_check_phones['d'] = date_str
    #     final_check_phones.to_parquet(f'{utils_path}/valid_phone.parquet',
    #                                   filesystem=hdfs,
    #                                   index=False,
    #                                   partition_cols='d')
    ...
    new_check_emails = check_emails[
        ~check_emails["email_raw"].isin(latest_check_emails["email_raw"].unique())
    ]
    new_check_phones = check_phones[
        ~check_phones["phone_raw"].isin(latest_check_phones["phone_raw"].unique())
    ]

    if new_check_emails.empty == False:
        email_input = (
            new_check_emails[["email_raw"]]
            .rename(columns={"email_raw": "email"})
            .copy()
        )
        ...
        new_check_emails["username_iscertain"] = new_check_emails[
            "username_iscertain"
        ].fillna(False)
        new_check_emails["is_autoemail"] = new_check_emails["is_autoemail"].fillna(
            False
        )
        new_check_emails["export_date"] = today

        latest_check_emails = pd.concat(
            [latest_check_emails, new_check_emails], ignore_index=True
        )
        latest_check_emails.loc[
            latest_check_emails["is_email_valid"] == False, "username_iscertain"
        ] = False
    ...
    if new_check_phones.empty == False:
        ...
        latest_check_phones = pd.concat(
            [latest_check_phones, new_check_phones], ignore_index=True
        )

    # SAVE (lastest)
    latest_check_emails.loc[
        latest_check_emails["customer_type"] != "Ca nhan", "gender"
    ] = None
    ...
    pro_check_emails = latest_check_emails[
        (latest_check_emails["is_email_valid"] == True)
    ]
    ...
    # phones
    pro_check_phones = latest_check_phones[
        latest_check_phones["is_phone_valid"] == True
    ]
...
# def MetaDataPhone(date_str):
#     utils_path = '/data/fpt/ftel/cads/dep_solution/sa/dev/utils'

#     # LOAD DATA
#     valid_phone = pd.read_parquet(f'{utils_path}/valid_phone.parquet/d={date_str}', filesystem=hdfs)
#     valid_phone = valid_phone.drop(columns=['phone_type', 'phone_vendor']).drop_duplicates()
...
#     # phone_type
#     valid_phone.loc[valid_phone['phone'].str[:3] == '024', 'phone_type'] = 'landline'
#     valid_phone.loc[valid_phone['phone'].str[:4].isin(df_subtele_vn['sub_phone'].unique()), 'phone_type'] = 'landline'
...
#     valid_phone_landline['sub_phone'] = valid_phone_landline['phone'].str[:4]
...
#     # remove folder
#     hdfs.delete_dir(f'{utils_path}/valid_phone.parquet/d={date_str}')

#     # return
#     valid_phone['d'] = date_str
#     valid_phone.to_parquet(f'{utils_path}/valid_phone.parquet',
#                            filesystem=hdfs,
#                            index=False,
#                            partition_cols='d')
