...
sys.path.append(
    '/bigdata/fdp/cdp/cdp_pages/scripts_hdfs/pre/utils/email2info/')
...
SUB_PHONE_11NUM = [x for x in sorted(
    subphone_vn['OldSubPhone'].unique()) if len(x) == 4]
SUB_TELEPHONE = sorted(df_subtele_vn['MaVung'].unique())
DICT_4SUBPHONE = subphone_vn[subphone_vn['OldSubPhone'].map(
    lambda x: len(x)) == 4].set_index('OldSubPhone').to_dict()['NewSubPhone']
EMAIL_CONFIGS = {
    'gmail': {
...
def check_valid_phone(f_phones: pd.DataFrame, phone_col: str = 'phone') -> pd.DataFrame:
    ...
    f_phones[phone_col] = f_phones[phone_col].str.replace(
        r'[^0-9]', '', regex=True).str.strip().str.replace('\s+', '', regex=True)
...
def check_valid_email(f_emails: pd.DataFrame, email_col: str = 'email') -> pd.DataFrame:
    ...
    f_emails[email_col + '_convert'] = f_emails[email_col].str.lower(
    ).str.strip().str.replace('\s+', '', regex=True).replace(repls)
...
def _validate_phone_email(df_profile: pd.DataFrame, email_col: str = 'email', phone_col: str = 'phone') -> pd.DataFrame:
    ...
    df_profile.drop(
        columns=['email_name', 'email_group', 'email_len'], inplace=True)
    df_profile.loc[
...
def validate_phone_email(df_profile: pd.DataFrame, n_cores: int, email_col: str = 'email', phone_col: str = 'phone') -> pd.DataFrame:
    if n_cores > 1:
        ...
        try:
            ...
            df_profile_val = pd.concat(pool.map(partial(
                _validate_phone_email, email_col=email_col, phone_col=phone_col), df_splits))
...
def MetaDataPhone(valid_phone):
    ...
    # phone_type
    valid_phone.loc[valid_phone['phone'].str[:3]
                    == '024', 'phone_type'] = 'landline'
    valid_phone.loc[valid_phone['phone'].str[:3].isin(
        df_subtele_vn['sub_phone'].unique()), 'phone_type'] = 'landline'
    ...
    valid_phone_mobile = valid_phone[valid_phone['phone_type']
                                     == 'mobile phone'].copy()
    valid_phone_none = valid_phone[valid_phone['phone_type'].isna()].copy()

    valid_phone_landline['sub_phone'] = valid_phone_landline['phone'].str[:4].astype(
        str)
    ...
    valid_phone.loc[valid_phone['tail_phone_type'].isna(
    ) & valid_phone['phone'].notna(), 'tail_phone_type'] = 'Số Thường'
...
def FormatName(valid_email):
    ...
    valid_email = valid_email.drop(
        columns=['customer_type', 'customer_type_detail'])

    # customer_type
    name_email = valid_email[valid_email['username'].notna()][[
        'username']].copy().drop_duplicates()
    ...
    per_name_email = name_email[~name_email['name'].isin(
        com_name_email['name'].unique())].copy()[['name']]
    ...
    per_name_email['format_name'] = per_name_email[columns].fillna('').agg(' '.join, axis=1).str.replace(
        '(?<![a-zA-Z0-9]),', '', regex=True).str.replace('-(?![a-zA-Z0-9])', '', regex=True)
    per_name_email['format_name'] = per_name_email['format_name'].str.strip(
    ).replace(dict_trash)
    ...
    # Fill Accent + gender
    map_name = valid_email[valid_email['customer_type']
                           == 'Ca nhan'][['username']].drop_duplicates().copy()
    map_name = fill_accent(map_name, 'username')
    map_name = map_name[map_name['is_human']
                        == True][['username', 'predict_name']]
    ...
    valid_email.loc[valid_email['predict_name'].isna(
    ), 'predict_name'] = valid_email['username']
    valid_email['username'] = valid_email['predict_name']

    valid_email.loc[valid_email['predict_gender'].isna(
    ), 'predict_gender'] = valid_email['gender']
...
def ValidPhoneEmail(date_str):
    ...
    for cttv, key in dict_cttv.items():
        emails_bank = emails_bank.append(
...
#         emails_bank = emails_bank.append(f_profile, ignore_index=True)
    ...
    # Regex syntax
    emails_bank['email'] = emails_bank['email'].str.strip(
...
#     raw_f_emails = check_valid_email(emails_bank.copy(), 'email')
    ...
    email_fo = profile_fo[['name', 'email',
                           'user_login', ]].drop_duplicates().copy()
    ...
    email_fo.loc[email_fo['name'].str.contains(
        '^[0-9]*$', case=False, na=False), 'name'] = None
    ...
    stats_email = stats_email[~stats_email['email'].isin(
        email_ok_cdp['email'])]
    ...
    stats_temp_profile_fo = temp_profile_fo.groupby(
        by=['cookie'])['vne_id'].agg(count_vne_id='count').reset_index()

    # cùng lúc, 1 cookie nhiều account
    cookies_chaos = stats_temp_profile_fo[stats_temp_profile_fo['count_vne_id'] > 1]['cookie'].tolist(
    )
    email_chaos_fo = temp_profile_fo[temp_profile_fo['cookie'].isin(
        cookies_chaos)]['email'].tolist()
    ...
    email_fplay.loc[email_fplay['name'].str.contains(
        '^[0-9]*$', case=False, na=False), 'name'] = None

    email_fplay['username_email'] = email_fplay['email'].str.split('@').str[0]
    email_fplay.loc[email_fplay['username_email'].str.contains(
        '^[0-9]*$', case=False, na=False), 'username_email'] = None
    email_fplay = email_fplay[email_fplay['username_email'].notna(
    )].drop_duplicates()
    email_fplay.loc[email_fplay['name'].isna(
    ), 'name'] = email_fplay.loc[email_fplay['name'].isna(), 'username_email']
    ...
    email_ok_fplay = email_ok_fplay[~email_ok_fplay['email'].isin(
        email_ok_cdp['email'])]
    ...
    # Domain email
    raw_stats_email['domain_email'] = raw_stats_email['email'].str.split(
        '@').str[1]
    stats_domain_email = raw_stats_email.groupby(by=['domain_email'])[
        'email'].agg(count_email='count').reset_index()
    bugs_domain_email = stats_domain_email[stats_domain_email['count_email']
                                           == 1]['domain_email'].tolist()
    ...
    email_warning_domain['email'] = stats_email[stats_email['domain_email'].isin(
        bugs_domain_email)]['email'].tolist()
    email_warning_domain['description'] = 'Warning - Domain email'
    email_warning_domain = email_warning_domain[~email_warning_domain['email'].isin(
        email_warning_cdp['email'])]

    email_warning_cdp = email_warning_cdp.append(
        email_warning_domain, ignore_index=True)
    stats_email = stats_email[~stats_email['email'].isin(
        email_warning_cdp['email'])].copy()
    ...
    email_extract = email_dict[email_dict['username'].notna()
                               ]['email'].tolist()
    ...
    temp_ok_email_extract = temp_ok_email_extract[~temp_ok_email_extract['email'].isin(
        email_ok_cdp['email'])]

    email_ok_cdp = email_ok_cdp.append(
        temp_ok_email_extract, ignore_index=True)
    stats_email = stats_email[~stats_email['email'].isin(
        email_ok_cdp['email'])].copy()
    ...
    email_ok_backup = email_ok_backup[~email_ok_backup['email'].isin(
        email_ok_cdp['email'])]
    ...
    final_stats_email = email_warning_cdp.append(
        email_ok_cdp, ignore_index=True)
    ...
    check_emails = raw_mapping_emails.merge(
        check_emails, how='left', on=['email'])
    check_emails.loc[check_emails['is_email_valid'].isna(),
    ...
    for cttv, key in dict_cttv.items():
        phones_bank = phones_bank.append(
            load_phone(cttv, key), ignore_index=True)
...
#         phones_bank = phones_bank.append(f_profile, ignore_index=True)
    ...
    phones_bank = phones_bank[phones_bank['phone'].notna()][[
...
#                                   partition_cols='d')
    ...
    new_check_emails = check_emails[~check_emails['email_raw'].isin(
        latest_check_emails['email_raw'].unique())]
    new_check_phones = check_phones[~check_phones['phone_raw'].isin(
        latest_check_phones['phone_raw'].unique())]

    if new_check_emails.empty == False:
        email_input = new_check_emails[['email_raw']].rename(
            columns={'email_raw': 'email'}).copy()
        email_output = email_get_info.pipeline_email_to_info(
            email_input.copy())
        email_to_info = email_input.rename(columns={'email': 'email_raw'})
        email_to_info['email'] = email_to_info['email_raw'].str.lower()
        email_to_info = email_to_info.merge(
            email_output, how='left', on=['email'])
        ...
        latest_check_emails.loc[latest_check_emails['is_email_valid']
                                == False, 'username_iscertain'] = False
    ...
    # SAVE (lastest)
    latest_check_emails.loc[latest_check_emails['customer_type']
                            != 'Ca nhan', 'gender'] = None
    ...
    columns_email = ['email', 'username_iscertain', 'username', 'year_of_birth', 'phone', 'address',
