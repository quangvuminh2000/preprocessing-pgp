...
class Level1toLevel3Parsing:
    def __init__(self, ):
        self.lv1_method = ['lv1_norm', 'lv1_abbrev',
                           'lv1_prefix_im', 'lv1_nprefix_im']
        self.lv2_method = ['lv2_norm', 'lv2_abbrev',
                           'lv2_prefix_im', 'lv2_nprefix_im']
        self.lv3_method = ['lv3_norm', 'lv3_abbrev',
                           'lv3_prefix_im', 'lv3_nprefix_im']

    def validate_found_level(self, ref_dict, found_item, validate_item, found_format, validate_format):
        check_df = ref_dict[ref_dict[found_format] == found_item][[
            found_format, validate_format]].drop_duplicates()
    ...
    def clean_address(self, df, **kwargs):
        ...
        df[clean_col] = df[input_col].str.replace(
            '\s+', ' ', regex=True).str.lower().apply(unidecode.unidecode)
        abbrev_list = ['tp.', 'tt.', 'q.', 'h.', 'x.', 'p.']
        norm_list = ['tp ', 'tt ', 'q ', 'h ', 'x ', 'p ']
        for case_abbrev, case_norm in zip(abbrev_list, norm_list):
            df[clean_col] = df[clean_col].str.replace(
                case_abbrev, case_norm, regex=False)
        df[clean_col] = df[clean_col].str.replace('\s+', ' ', regex=True)
        df[clean_col] = df[clean_col].apply(
            lambda x: re.sub(r"(\d+)", pad_number, x))
        df[clean_col] = df[clean_col].apply(self.prefix_loc_number)
        df[clean_col] = df[clean_col].str.replace(
            'ba ria vung tau', 'ba ria - vung tau', regex=False)
        df[clean_col] = df[clean_col].str.replace(
            'phan rang thap cham', 'phan rang - thap cham', regex=False)
        return df
...
#         print('found_1',found_1)
        ...
        kw_proc_2 = self.get_keywordprocess(
            format_2, ref_dict_2.copy(), level_start_from)

        found_2 = self.pat_matching(query, kw_proc_2)
#         print('found_2',found_2)
        query = rreplace(query, found_2)
        check_found_2 = self.validate_found_level(
            ref_dict, found_2, found_1, format_2, format_1)
        ...
        ref_dict_3 = ref_dict[(ref_dict[format_1] == found_1) & (
            ref_dict[format_2] == found_2)]
        kw_proc_3 = self.get_keywordprocess(
            format_3, ref_dict_3.copy(), level_start_from)

        for case in ['q ', 'tp ']:
            query = rreplace(query, case)
        found_3 = self.pat_matching(query, kw_proc_3)
#         print('found_3',found_3)
        check_found_3 = self.validate_found_level(
            ref_dict, found_3, found_1, format_3, format_1)
        ...
        if is_ok >= num_lv_req:
            if num_lv_req == 3:
                format_dict = ref_dict[(ref_dict[format_1] == found_1) & (
                    ref_dict[format_2] == found_2) & (ref_dict[format_3] == found_3)]
            elif num_lv_req == 2:
                format_dict = ref_dict[(ref_dict[format_1] == found_1) & (
                    ref_dict[format_2] == found_2)]
            elif num_lv_req == 1:
            ...
            if len(found_3) > 1:
                lv3_from_lv1_2 = (format_dict[format_1_norm] == found_1) & (
                    format_dict[format_2_norm] == found_2)
    ...
    def get_pat_dict(self, df_dictionary, lv_method, lv_start_from):
        ...
        if level_name == lv_start_from:
            df_unique_dict = df_dictionary[[
                level_name, lv_method]].drop_duplicates()
        else:
            df_unique_dict = df_dictionary[[lv_method]].drop_duplicates()

        pat_finding = df_unique_dict[~df_unique_dict[lv_method].duplicated(
            keep=False)][lv_method].unique().tolist()
        return pat_finding

    def get_keywordprocess(self, lv_method, ref_dict, lv_start_from):
        pat_finding = self.get_pat_dict(
            ref_dict.copy(), lv_method, lv_start_from)
    ...
    def get_full_level(self, df_data, **kwargs):
        ...
        for order_lv_extract in order_search:
            ...
            kw_processor1 = self.get_keywordprocess(
                format1, ref_dict.copy(), level_start_from)
            ...
            map_address = pd.Series(
                tmp_df[output_col1].values, index=tmp_df[input_col]).to_dict()
            update_condition = df_data[output_col1].isna()
            df_data.loc[update_condition, output_col1] = df_data.loc[update_condition, input_col].map(
                map_address)
            update_method = update_condition & (df_data[output_col1].notna())
            df_data.loc[update_method, output_col2] = '%s|%s|%s' % (
                order_lv_extract[0], order_lv_extract[1], order_lv_extract[2])
    ...
    def post_process(self, df, **kwargs):
        ...
        for val_col, order_col in zip(temp_val_df.columns, temp_order_df.columns):
            ...
            temp_val_df.loc[temp_val_df[val_col].str.len() < 2,
                            val_col] = np.nan
    #         print('temp_order_df',temp_order_df)
            for lv_col in ['lv1', 'lv2', 'lv3']:
                df.loc[temp_order_df['level_col_name'] == lv_col,
                       lv_col] = temp_val_df.loc[temp_order_df['level_col_name'] == lv_col, val_col]
    ...
    def mapping_results(self, df, df_parsed, **kwargs):
        ...
        input_col = kwargs['input_col']
        order_decode = kwargs['order_decode']
        address_available = df_parsed[df_parsed[input_col].notna()].reset_index(
            drop=True)
        ...
        map_add_lv1 = pd.Series(
            mapping_df['lv1'].values, index=mapping_df[raw_col_str]).to_dict()
        map_add_lv2 = pd.Series(
            mapping_df['lv2'].values, index=mapping_df[raw_col_str]).to_dict()
        map_add_lv3 = pd.Series(
            mapping_df['lv3'].values, index=mapping_df[raw_col_str]).to_dict()

        df.loc[df['lv1'].isna(), 'lv1'] = df.loc[df['lv1'].isna(),
                                                 raw_col_str].map(map_add_lv1)
        df.loc[df['lv2'].isna(), 'lv2'] = df.loc[df['lv2'].isna(),
                                                 raw_col_str].map(map_add_lv2)
        df.loc[df['lv3'].isna(), 'lv3'] = df.loc[df['lv3'].isna(),
                                                 raw_col_str].map(map_add_lv3)
        for col in ['lv1', 'lv2', 'lv3']:
            if df.loc[df[col].notna()].shape[0] == 0:
                continue
            df.loc[df[col].notna(), col] = df.loc[df[col].notna(),
                                                  col].str.strip().apply(unidecode.unidecode)
    ...
    def get_2lvs_dict(self):
        ...
        order_2lvs_1 = list(itertools.product(*from_lv1_1)) + list(
            itertools.product(*from_lv2_1)) + list(itertools.product(*from_lv3_1))
        order_2lvs_2 = list(itertools.product(*from_lv1_2)) + list(
            itertools.product(*from_lv2_2)) + list(itertools.product(*from_lv3_2))
    ...
    def get_1lv_dict(self):
        ...
        order_1lv = list(itertools.product(
            *from_lv1)) + list(itertools.product(*from_lv2)) + list(itertools.product(*from_lv3))
        return [order_1lv]

    def set_lv_method(self, lv, lv_form):
        if lv == 'level1':
    ...
    def infer_otherloc_from1lv(self, df, pre_lv, lv, ref_dict):
        ...
        map_lv = pd.Series(
            df_uniq_rel[pre_lv].values, index=df_uniq_rel[lv]).to_dict()
        condition = (df[pre_lv].isna()) & (df[lv].notna())
        df.loc[condition, pre_lv] = df.loc[condition, lv].map(map_lv)
        df.loc[condition & (df[pre_lv].notna()),
               'reverse'] = '%s-%s' % (lv, pre_lv)
        return df
    ...
    def infer_otherloc_from2lvs(self, df, pre_lv, lv1str_infer, lv2str_infer, ref_dict):
        ...
        ref_dict[lv] = ref_dict[lv1str_infer].str.strip(
        ) + ref_dict[lv2str_infer].str.strip()
        df_uniq_rel = ref_dict[[pre_lv, lv]].drop_duplicates()
        df_uniq_rel = df_uniq_rel[~df_uniq_rel[lv].duplicated(keep=False)]
        map_lv = pd.Series(
            df_uniq_rel[pre_lv].values, index=df_uniq_rel[lv]).to_dict()
    ...
    def handle_outlier_case(self, df, num_lv_parsed_col, ref_dict):
        ...
        for infer_from, infer_to in zip(infer_from_list, infer_to_list):
            df_infer_reversed = self.infer_otherloc_from1lv(
                df_infer_1_lv.copy(), infer_from, infer_to, ref_dict)
        ...
        for infer_from1, infer_from2, infer_to in zip(infer2lvs_frlist_1, infer2lvs_frlist_2, infer2lvs_tolist):
            ...
            df_infer = self.infer_otherloc_from2lvs(
                df_infer.copy(), infer_to, infer_from1, infer_from2, ref_dict)

        thuduc_case = ref_dict[ref_dict['lv2'] ==
                               'Thanh pho Thu Duc']['lv3'].unique().tolist()
...
class Level4Parsing:
    ...
    def clean_data(self, df, **kwargs):
        ...
        redundant_space_case = ['P. ', 'p. ',
                                'H. ', 'h. ', 'X. ', 'x.', 'T. ', 't. ']
        correct_case = ['P.', 'p.', 'H.', 'h.', 'X.', 'x.', 'T.', 't.']
        for case_redundant, case_correct in zip(redundant_space_case, correct_case):
            df[clean_col] = df[clean_col].str.replace(
                case_redundant, case_correct, regex=False)

        abbrev_case = ['tp.', 'q.', 'p.', ' d.',
                       ' đ ', 'đ. ', 'kp', 'kkt', 'kcn']
        norm_case = ['thanh pho ', 'quan ', 'phuong ', ' duong ',
                     ' duong ', ' duong ', 'khu pho', 'khu kinh te', 'khu cong nghiep']

        for abbrev, norm in zip(abbrev_case, norm_case):
            df[clean_col] = df[clean_col].str.replace(
                abbrev, norm, regex=False)
    ...
    def post_process(self, df, col2proc):
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace('[,;!@#%&.]', ' ').str.title()

        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace('\s+', ' ', regex=True)
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace(' - ', '-', regex=False)
        for case in ["P;", "T;", "X;", 'Dia chi', "N'", ":"]:
            df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                            col2proc].str.replace(case, "", regex=False)

        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(
        ), col2proc].str.replace(r'[^[a-zA-Z0-9| |,|-|.|()|/]]', ' ')
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace('()', '', regex=False)
        ...
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.strip()
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace(' (P|X|Tt|Tx|Q|H|Tp)$', '')
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(
        ), col2proc].apply(lambda x: re.sub('(/|-)+[^\d]', self.match_spec_char, x))
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(
        ), col2proc].apply(lambda x: re.sub('[(].*[^)]$', '', x))
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace('(/|-){1,}$', '')
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.replace('\s+', ' ', regex=True)
        df.loc[df[col2proc].notna(), col2proc] = df.loc[df[col2proc].notna(),
                                                        col2proc].str.strip()
