...
def get_hdfs_profile_raw(data_datetime,
    ...
    profile_data_hdfs = profile_data_hdfs.rename(columns={raw_address_col:new_address_col})
...
#     df_prof_fin = None
    ...
    for address_name in address_name_list:
        ...
        try:
            ...
            new_address_df = df_read_hdfs[~df_read_hdfs[idcol_new].isin(latest_address_df)].reset_index(drop=True)
            
            if new_address_col not in new_address_df.columns:
                print(f'Address column {new_address_col} not found in {address_name} raw data')
                continue
                
            else:
                new_address_hdfs = new_address_df[[idcol_new, new_address_col]].drop_duplicates()
            new_address_total_hdfs = pd.concat([new_address_total_hdfs, new_address_hdfs]).drop_duplicates()
...
def get_lv1_to_lv3(df, ref_dict, loc_parse_module,
    ...
    for required_lvs in [3, 2, 1]:
        ...
        df_input.loc[(df_input[raw_col_str].isin(modified_address)) & (df_input['lvs_parsed'].isna()), 'lvs_parsed'] = required_lvs
    ...
    for lv_col in ['lv1', 'lv2', 'lv3']:
        outlier_address_dict = pd.Series(df_outlier[lv_col].values, index=df_outlier[raw_col_str]).to_dict()
        df_input.loc[df_input[raw_col_str].isin(df_outlier[raw_col_str].unique().tolist()), lv_col] =\
        df_input.loc[df_input[raw_col_str].isin(df_outlier[raw_col_str].unique().tolist()),raw_col_str].map(outlier_address_dict)
    return df_input
...
def get_lv4(df, lv_col, location_dict_data, raw_address_col_str, clean_address_col_str):
    ...
    df_cleanadd = my_loc_lv4.clean_data(df_address.copy(), input_col = raw_address_col_str, clean_col = clean_address_col_str)
    ...
    df_cleanadd.loc[df_cleanadd[clean_address_col_str].notna()].apply(lambda x: my_loc_lv4.clip_street_address(x[clean_address_col_str], x.lv3, x.lv2, x.lv1), axis=1)
    ...
    map_lv4 = pd.Series(df_lv4_output.lv4.values, index=df_lv4_output[raw_address_col_str]).to_dict()
    df.loc[df[raw_address_col_str].isin(df_lv4_output[raw_address_col_str].unique().tolist()), lv_col] =\
    df.loc[df[raw_address_col_str].isin(df_lv4_output[raw_address_col_str].unique().tolist()),raw_address_col_str].map(map_lv4)
...
#                        'district':'District', 'city':'City', 'Update':'Update'})
    ...
    df_address_update = df[~df[id_col_str].isin(df_dict[id_col_str])].drop_duplicates().reset_index(drop=True)
    ...
    for map_col in [address_norm_col_str, address_lv4_col_str, address_lv3_col_str,
        ...
        map_dict = pd.Series(address_dict[map_col].values, index=address_dict[address_input_col_str]).to_dict()
        df_address_update[map_col] = df_address_update[address_input_col_str].map(map_dict)
    df_address_update[update_col_str] = df_address_update[update_col_str].fillna(datetime.today())
    print('%s: %d new address updated in %s'%(cttv_str, df_address_update.shape[0], datetime.today().strftime('%Y-%m-%d')))    

    df_out = pd.concat([df_dict, df_address_update])[OUT_COLS].drop_duplicates().reset_index(drop=True)
...
if __name__ == "__main__":
    ...
    if df_new_address_raw is None:
        ...
        df_new_address = df_new_address_raw[df_new_address_raw[address_col_str].str.len() >5].reset_index(drop=True)
        ...
        df_clean_address = my_loc_parse.clean_address(df_new_address.copy(), address_col_str=address_col_str, clean_col_str=address_clean_col_str)
        ...
        df_full_lv = get_lv4(df_3lvs.copy(), 'lv4', location_df.copy(), address_col_str, address_clean_col_str)
        ...
        if longchau_latest_address_output is not None:
            print(f'Saving Long Chau process address to {longchau_proc_address_outf}')
