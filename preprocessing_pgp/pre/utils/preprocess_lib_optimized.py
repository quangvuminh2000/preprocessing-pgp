...
def BuildWordName():
    ...
    ext_data_name1 = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/ext_data.parquet",
        filesystem=hdfs,
    )
    ext_data_name2 = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/ext_data_uit.parquet",
        filesystem=hdfs,
    )
...
def BuildLastName():
    ...
    stats_lastname_vn = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/stats_lastname_vn.parquet",
        filesystem=hdfs,
    )
        ...
        while check_end_case1 == False:
            for key_vi in last_name_list1:
                ...
                if (full_name.split(" ") == 1) or (
                    key_vi.strip() == last_name_list1[-1]
                ):
                    check_end_case1 = True
        ...
        if last_name.strip() == "":
            ...
            while check_end_case2 == False:
                for key_vi in last_name_list2:
                    ...
                    if (full_name.split(" ") == 1) or (
                        key_vi.strip() == last_name_list2[-1]
                    ):
                        check_end_case2 = True
        ...
        if last_name.strip() == "":
            ...
            while check_end_case3 == False:
                for key_vi in last_name_list3:
                    ...
                    if (full_name.split(" ") == 1) or (
                        key_vi.strip() == last_name_list3[-1]
                    ):
                        check_end_case3 = True
...
def RunExtractCustomerType(
    ...
    profile_opt["{}_en_clean".format(name_col)] = profile_opt[
        "{}_en".format(name_col)
    ...
    def kw_in_lv2(name):
        ...
        return [lv2 for lv2 in lv2_kws if lv2.lower() in name.lower()]
    ...
    profile_after = pd.concat([profile, profile_opt], ignore_index=True)
...
def ExtractCustomerType(profile):
    ...
    profile["{}_en".format(name_col)] = profile[name_col].apply(
        lambda name: unidecode(
            unicodedata.normalize("NFKD", " ".join(name.split()))
        ).lower()
    ...
    profile_opt = profile.assign(customer_type=None).copy()
        ...
        profile_opt = pd.concat(
            [
                RunExtractCustomerType(
                    ...
                    lv2_kws,
                    lv2_kws_map,
                    exclude_regex=EXCLUDE_REGEXES[ctype],
                ),
                profile_exclude,
            ],
            ignore_index=True,
    ...
    profile_opt_final = profile_opt.loc[
        :, ["name", "customer_type"]
...
def Name2Gender(df, name_col="name"):
    ...
    # load model
    accented_model = pickle.loads(
        hdfs.open_input_file(
            f"/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/accented/logistic_pipeline.pkl"
        ).read()
    )
    non_accented_model = pickle.loads(
        hdfs.open_input_file(
            f"/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/not_accented/logistic_pipeline.pkl"
        ).read()
    )
    ...
    if accented_process_df.empty == False:
        ...
        process_df.loc[
            accented_process_df.index, "predict_gender"
        ] = accented_predict_gender
    ...
    if non_accented_process_df.empty == False:
        ...
        process_df.loc[
            non_accented_process_df.index, "predict_gender"
...
def UpdateDictName(date_str: str, n_cores: int = 1):
    # LOAD DATA
    # dict name
    ...
    for name_cttv in cttv_s:
        ...
        # filters
        data_cttv = data_cttv[data_cttv["name"].notna()].drop_duplicates(
            subset=["name"]
        )
    ...
    name_email = pd.read_parquet(
        "/data/fpt/ftel/cads/dep_solution/sa/cdp/data/email_dict_latest.parquet",
        filesystem=hdfs,
        filters=[("username_iscertain", "=", True)],
    )[["username", "gender"]]
    name_email = name_email[name_email["username"].notna()].drop_duplicates()
    name_email.rename(columns={"username": "name"}, inplace=True)
    ...
    pre_names = process_enrich(raw_names, name_col="name", n_cores=n_cores)
    ...
    pre_names[confused_gender_mask, "is_confused"] = True
    ...
    dict_name_cdp.to_parquet(
        ROOT_PATH + "/utils/dict_name_latest_new.parquet", filesystem=hdfs, index=False
    )
    ...
    pro_dict_name_cdp.to_parquet(
        "/data/fpt/ftel/cads/dep_solution/sa/cdp/data/dict_name_latest_new.parquet",
        filesystem=hdfs,
        index=False,
    )
...
def FindUniqueName(
    ...
    for name_cttv, info_columns in priority_names.items():
        ...
        data = pd.read_parquet(
            f"{raw_path}/{name_cttv}.parquet/d={date_str}",
            filters=[(key, "in", value_valid_key)],
            columns=[key] + info_columns,
            filesystem=hdfs,
        )
    ...
    stats_best_name = (
        dup_name_gender_by_key.groupby(by=[key, "best_name"])["best_name"]
    ...
    dup_name_gender_by_key = dup_name_gender_by_key.sort_values(
        by=[key, "last_active", "active_date", "num_overall", "priority"],
    ...
    com_dup_name_gender_by_key["num_word"] = (
        com_dup_name_gender_by_key["raw_name"].str.split(" ").str.len()
    )
    com_dup_name_gender_by_key["num_char"] = com_dup_name_gender_by_key[
        "raw_name"
    ].str.len()
    ...
    com_unique_name_gender_by_key = (
        com_dup_name_gender_by_key.groupby(by=[key])
    ...
    per_dup_name_gender_by_key["num_word"] = (
        per_dup_name_gender_by_key["raw_name"].str.split(" ").str.len()
    )
    per_dup_name_gender_by_key["num_char"] = per_dup_name_gender_by_key[
        "raw_name"
    ].str.len()
    per_dup_name_gender_by_key = per_dup_name_gender_by_key.sort_values(
        by=[
            key,
            "last_active",
            "active_date",
    ...
    per_dup_name_gender_by_key = (
        per_dup_name_gender_by_key.groupby(by=[key])
        .head(1)[[key, "best_name", "source_best_name"]]
        .drop_duplicates()
    )
    ...
    only_name_gender_by_key["source_unique_name"] = only_name_gender_by_key[
        "source_name"
    ]
...
def DictNameGender():
    ...
    data_name_cdp.loc[data_name_cdp["gender"].notna(), "source_gender"] = data_name_cdp[
        "source_name"
    ]
    ...
    data_name_cdp = pd.concat(
        [data_name_cdp_gender, data_name_cdp_non_gender], ignore_index=True
    ...
    data_name_cdp.loc[data_name_cdp["gender"].isna(), "gender"] = data_name_cdp[
        "predict_gender"
    ]

    # PostProcess
    customer_name = ExtractCustomerType(data_name_cdp)
    com_name = customer_name[
        customer_name["customer_type"].notna()
        | (customer_name["name"].str.split(" ").str.len() > 5)
    ]["name"].unique()
