...
def BuildWordName():
    # load database name
    ext_data_name1 = pd.read_parquet('/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/ext_data.parquet', filesystem=hdfs)
    ext_data_name2 = pd.read_parquet('/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/ext_data_uit.parquet', filesystem=hdfs)

    ext_data_name1.columns = ['full_name', 'gender'] 
    ext_data_name1['full_name'] = ext_data_name1['full_name'].str.replace('\s+', ' ', regex=True).str.title()
    ext_data_name1['first_name'] = ext_data_name1['full_name'].str.split(' ').str[-1]
    ext_data_name1['last_name_group'] = ext_data_name1['full_name'].str.split(' ').str[0]
    ext_data_name1['last_name'] = ext_data_name1['full_name'].str.split(' ').str[:-1].str.join(' ')

    ext_data_name2['full_name'] = ext_data_name2['full_name'].str.replace('\s+', ' ', regex=True).str.title()
    ext_data_name2 = ext_data_name2[['full_name', 'gender', 'first_name', 'last_name_group', 'last_name']].copy()

    # stats freq
    ext_data_name1['full_name_unicecode'] = ext_data_name1['full_name'].str.lower().apply(unidecode)
    stats_word_name1 = ext_data_name1['full_name_unicecode'].str.split(expand=True).stack().value_counts().reset_index()
    stats_word_name1.columns = ['Word', 'Frequency'] 

    ext_data_name2['full_name_unicecode'] = ext_data_name2['full_name'].str.lower().apply(unidecode)
    stats_word_name2 = ext_data_name2['full_name_unicecode'].str.split(expand=True).stack().value_counts().reset_index()
    stats_word_name2.columns = ['Word', 'Frequency'] 

    # wordName
    stats_word_name = stats_word_name1.append(stats_word_name2, ignore_index=False)
    stats_word_name = stats_word_name.groupby(by=['Word'])['Frequency'].sum().reset_index()
...
def BuildLastName():
    # load stats lastname
    stats_lastname_vn = pd.read_parquet('/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/dataset/stats_lastname_vn.parquet', filesystem=hdfs)

    last_name_list1 = list(stats_lastname_vn[stats_lastname_vn['No'] <= 50]['Last_Name'].unique())
    ...
    last_name_list = list(set(last_name_list3) | set(last_name_list1) | set(last_name_list2))
        ...
        if (last_name.strip() == ''):
            ...
            while (check_end_case2 == False):
                for key_vi in last_name_list2:
                    ...
                    is_case21 = (len(full_name)-full_name.rfind(key_vi) == len(key_vi)) & (full_name.rfind(key_vi) != -1)
                    is_case22 = (len(full_name)-full_name.rfind(unidecode(key_vi)) == len(unidecode(key_vi))) & (full_name.rfind(unidecode(key_vi)) != -1)
...
def RunExtractCustomerType(profile,
    ...
    conditions = profile_opt['{}_en'.format(name_col)].str.contains('|'.join([unidecode(kw) for kw in lv1_kws]), regex=True, na=False, case=False)
    if exclude_regex:
        conditions = conditions & ~profile_opt['{}_en'.format(name_col)].str.contains(exclude_regex, regex=True, na=False, case=False)
    
    profile = profile[~conditions]
    profile_opt = profile_opt[conditions]
    profile_opt['{}_en_clean'.format(name_col)] = profile_opt['{}_en'.format(name_col)].str.replace(rf'[{punctuation}]', '', regex=True)
...
def ExtractCustomerType(profile):
    ...
    profile['{}_en'.format(name_col)] = profile[name_col].apply(lambda name: unidecode(unicodedata.normalize('NFKD', ' '.join(name.split()))).lower() if isinstance(name, str) else name)
    profile['{}_len'.format(name_col)] = profile['{}_en'.format(name_col)].apply(lambda name: len(name.split(' ')) if isinstance(name, str) else 0)
    ...
    for ctype in ['company', 'biz', 'edu', 'medical']:
        ...
        profile_opt = pd.concat([
            RunExtractCustomerType(profile_opt, CTYPE_NAMES[ctype], lv1_kws, lv2_kws, lv2_kws_map, exclude_regex=EXCLUDE_REGEXES[ctype]),
            profile_exclude,
        ], ignore_index=True)
...
def Name2Gender(df, name_col='name'):
    ...
    # check have_accented
    have_accented = (process_df['processed_name'].apply(unidecode) != process_df['processed_name'])
    
    # load model
    accented_model = pickle.loads(hdfs.open_input_file(f'/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/accented/logistic_pipeline.pkl').read())
    non_accented_model = pickle.loads(hdfs.open_input_file(f'/data/fpt/ftel/cads/dep_solution/user/namdp11/name2gender/not_accented/logistic_pipeline.pkl').read())
    
    # predict gender (accented)
    accented_process_df = process_df[have_accented].copy()
    if (accented_process_df.empty == False):
        accented_predict_gender = accented_model.predict(accented_process_df['processed_name'].values)
        process_df.loc[accented_process_df.index, 'predict_gender'] = accented_predict_gender
    
    # predict gender (non accented)
    non_accented_process_df = process_df[~have_accented].copy()
    if (non_accented_process_df.empty == False):
        non_accented_predict_gender = non_accented_model.predict(non_accented_process_df['processed_name'].values)
        process_df.loc[non_accented_process_df.index, 'predict_gender'] = non_accented_predict_gender
...
def UpdateDictName(date_str):
    # LOAD DATA
    # dict name
    dict_name_cdp = pd.read_parquet('/data/fpt/ftel/cads/dep_solution/sa/dev/utils/dict_name_latest.parquet', filesystem=hdfs)
    dict_name_cdp['unidecode_full_name'] = dict_name_cdp['full_name'].apply(unidecode)
    
    # Load data CDP
    pre_path = f'/data/fpt/ftel/cads/dep_solution/sa/dev/pre'
    ...
    for name_cttv in cttv_s:
        ...
        # filters
        data_cttv = data_cttv[data_cttv['name'].notna()][['name']].drop_duplicates()
    ...
    accent_raw_names = raw_names[raw_names['name'] != raw_names['unidecode_full_name']][['name']].copy()
    not_accent_raw_names = raw_names[raw_names['name'] == raw_names['unidecode_full_name']][['name']].copy()

    # NAME ACCENT
    # Add source
    accent_raw_names = accent_raw_names[~accent_raw_names['name'].isin(dict_name_cdp['full_name'])].copy()
    ...
    process_names = not_accent_raw_names[~not_accent_raw_names['name'].isin(old_names)]
    
    # Run: accent, split, gender
    print(f'{len(process_names)} names')
    process_names = fill_accent(process_names, 'name')
    process_names = process_names[process_names['is_human'] == True][['predict_name']].drop_duplicates()
    process_names.columns = ['full_name']
    ...
    with mp.Pool(8) as pool:
        pre_names[['last_name', 'middle_name', 'first_name']] = pool.map(SplitName, pre_names['full_name'])
    ...
    dict_name_cdp = dict_name_cdp.drop_duplicates(subset=['full_name'], keep='first')
    
    # Post-process
    # split data
    dict_name_cdp['unidecode_full_name'] = dict_name_cdp['full_name'].apply(unidecode)
    only_name = dict_name_cdp[dict_name_cdp['full_name'].str.split(' ').str.len() == 1].copy()
    full_name = dict_name_cdp[dict_name_cdp['full_name'].str.split(' ').str.len() != 1].copy()

    # post case only_name
    only_name = only_name[only_name['unidecode_full_name'].str.lower().isin(word_name)]
    ...
    # SAVE DEV
    dict_name_cdp.to_parquet('/data/fpt/ftel/cads/dep_solution/sa/dev/utils/dict_name_latest.parquet', filesystem=hdfs, index=False)
    
    # SAVE PRODUCT
    pro_dict_name_cdp = dict_name_cdp.drop(columns=['source', 'd'])
    pro_dict_name_cdp = pro_dict_name_cdp.drop_duplicates().sample(frac=1)

    pro_dict_name_cdp.to_parquet('/data/fpt/ftel/cads/dep_solution/sa/cdp/data/dict_name_latest.parquet', filesystem=hdfs, index=False)
...
def LoadNameGender(date_str, key='phone'):
    ...
    # drop duplicate
    name_gender = name_gender.drop_duplicates(subset=[key, 'name', 'customer_type'], keep='first')
    name_gender = name_gender.rename(columns={'name': 'raw_name'})
    
    
    # load name (with accent)
    dict_name_cdp = pd.read_parquet('/data/fpt/ftel/cads/dep_solution/sa/dev/utils/dict_name_latest.parquet', 
                                    filesystem=hdfs, columns=['full_name', 'd', 'source'])
    dict_name_cdp['unidecode_full_name'] = dict_name_cdp['full_name'].apply(unidecode)
    dict_name_cdp = dict_name_cdp.sort_values(by=['unidecode_full_name', 'source'], ascending=False)
#     dict_name_cdp = dict_name_cdp.drop_duplicates(subset=['unidecode_full_name'], keep='last')
    dict_name_cdp = dict_name_cdp[dict_name_cdp['source'] == 'enrich accent'].drop_duplicates(subset=['unidecode_full_name'], keep='first')
    ...
    name_gender.loc[name_gender['name_with_accent'].notna(), 'name'] = name_gender['name_with_accent']
...
def CoreBestName(raw_names_n, key='phone'):
    ...
    skip_names_df = raw_names_n[raw_names_n['raw_name'].isin(skip_names)][[key, 'raw_name']].copy().drop_duplicates()
    names_df = raw_names_n[~raw_names_n['raw_name'].isin(skip_names)][[key, 'raw_name']].copy().drop_duplicates()
    ...
    with mp.Pool(8) as pool:
        map_split_name[['last_name', 'middle_name', 'first_name']] = pool.map(SplitName, map_split_name['raw_name'])
    ...
    names_df.loc[names_df['last_name'].notna(), 'unidecode_last_name'] = names_df.loc[names_df['last_name'].notna(), 'last_name'].apply(unidecode)
    names_df['num_last_name'] = names_df.groupby(by=['group_id'])['unidecode_last_name'].transform('nunique')

    info_name_columns = ['group_id', 'raw_name', 'last_name', 'middle_name', 'first_name']
    ...
    map_names_n_df['accented'] = map_names_n_df['raw_name'] != map_names_n_df['raw_name'].apply(unidecode)
    map_names_n_df = map_names_n_df.sort_values(by=['group_id', 'num_word', 'num_char', 'accented'], ascending=False)
    map_names_n_df = map_names_n_df.groupby(by=['group_id']).head(1)
    map_names_n_df = map_names_n_df[['group_id', 'raw_name']].rename(columns={'raw_name': 'best_name'})
    ...
    names_n_df = names_n_df.merge(post_names_n_df, how='left', on=['group_id', 'raw_name'])
    ...
    for element_name in ['last_name', 'middle_name', 'first_name']:
        # filter data detail
        map_element_name = names_1_df[names_1_df[element_name].notna()][['group_id', element_name]].copy().drop_duplicates()

        # create features
        map_element_name[f'unidecode_{element_name}'] = map_element_name[element_name].apply(unidecode)
        map_element_name['num_overall'] = map_element_name.groupby(by=['group_id', f'unidecode_{element_name}'])[element_name].transform('count')
        ...
        map_element_name['num_word'] = map_element_name[element_name].str.split(' ').str.len()
        map_element_name['accented'] = map_element_name[element_name] != map_element_name[element_name].apply(unidecode)
        ...
        map_element_name = map_element_name.sort_values(by=['group_id', 'accented', 'num_overall', 'num_char', 'num_word'], ascending=False)
        ...
        # merge
        map_names_1_df = map_names_1_df.merge(map_element_name, how='left', on=['group_id'])
        map_names_1_df.loc[map_names_1_df[f'best_{element_name}'].isna(), f'best_{element_name}'] = None
    ...
    map_names_1_df['best_name'] = map_names_1_df[columns].fillna('').agg(' '.join, axis=1).str.replace('(?<![a-zA-Z0-9]),', '', regex=True).str.replace('-(?![a-zA-Z0-9])', '', regex=True)
    map_names_1_df['best_name'] = map_names_1_df['best_name'].str.strip().replace(dict_trash)
    map_names_1_df.loc[map_names_1_df['best_name'].isna(), 'best_name'] = None

    # merge
    names_1_df = names_1_df.merge(map_names_1_df[['group_id', 'best_name']], how='left', on=['group_id'])
    ...
    with mp.Pool(8) as pool:
        map_element_name[['last_name', 'middle_name', 'first_name']] = pool.map(SplitName, map_element_name['name'])

    for flag in ['raw', 'best']:
        temp = map_element_name.copy()
        temp.columns = [f'{flag}_name', f'{flag}_last_name', f'{flag}_middle_name', f'{flag}_first_name']
    ...
    for element_name in ['last_name', 'middle_name', 'first_name']:
        ...
        condition_compare = names_df[f'raw_{element_name}'].notna() & names_df[f'best_{element_name}'].notna()
        ...
        compare_names_df[f'similar_{element_name}'] = compare_names_df[f'raw_{element_name}'].apply(unidecode) == compare_names_df[f'best_{element_name}'].apply(unidecode)
        compare_names_df[f'similar_{element_name}'] = compare_names_df[f'similar_{element_name}'].astype(int)

        not_compare_names_df[f'similar_{element_name}'] = 1

        # concat
        names_df = pd.concat([compare_names_df, not_compare_names_df], ignore_index=True)

    weights = [0.25, 0.25, 0.5]
    names_df['simility_score'] = weights[0]*names_df['similar_last_name'] + weights[1]*names_df['similar_middle_name'] + weights[2]*names_df['similar_first_name']
    
    print(">> Simility_score")
    
    # Postprocess
    pre_names_df = names_df[['group_id', 'raw_name', 'best_name', 'simility_score']].copy()
    ...
    pre_names_df.loc[pre_names_df['best_name'].isna(), 'best_name'] = pre_names_df['raw_name']
    ...
    pre_names_n.loc[pre_names_n['best_name'].isna(), 'best_name'] = pre_names_n['raw_name']
    ...
    map_source_best_name = map_source_best_name.rename(columns={'source_name': 'source_best_name'})
    pre_names_n = pre_names_n.merge(map_source_best_name, how='left', on=[key, 'best_name'])
...
def FindUniqueName(name_gender_by_key, date_str, key='phone'):
    ...
    dup_name_gender_by_key = name_gender_by_key[name_gender_by_key[key].isin(dup_key_s)].copy()
    only_name_gender_by_key = name_gender_by_key[~name_gender_by_key[key].isin(dup_key_s)].copy()
    ...
    utils_path = '/data/fpt/ftel/cads/dep_solution/sa/dev/utils'

    raw_name = pd.DataFrame(columns=[key, 'raw_name', 'active_date', 'last_active', 'f_group', 'priority'])
    priority_index = 1
    for name_cttv, info_columns in priority_names.items():
        ...
        value_valid_key = valid_key[valid_key[f'{key}_raw'].notna()][f'{key}_raw'].unique()
    ...
    # clean name
    map_name['clean_name'] = map_name.apply(lambda row: CleanName(row['raw_name'], ''), axis=1)
    map_name = map_name[map_name['clean_name'].notna()]

    # format name
    dict_trash = {'': None, 'Nan': None, 'nan': None, 'None': None, 'none': None, 'Null': None, 'null': None, "''": None}
    with mp.Pool(8) as pool:
        map_name[['last_name', 'middle_name', 'first_name']] = pool.map(SplitName, map_name['clean_name'])
    element_name_columns = ['last_name', 'middle_name', 'first_name']
    map_name['clean_name'] = map_name[element_name_columns].fillna('').agg(' '.join, axis=1).str.replace('(?<![a-zA-Z0-9]),', '', regex=True).str.replace('-(?![a-zA-Z0-9])', '', regex=True)
    ...
    map_name_active = map_name_active.drop(columns=['clean_name', 'f_group']).drop_duplicates()
    
    # merge key
    dup_name_gender_by_key = dup_name_gender_by_key.merge(map_name_active, how='left', on=[key, 'raw_name'])
    stats_best_name = dup_name_gender_by_key.groupby(by=[key, 'best_name'])['best_name'].agg(num_overall='count').reset_index()
    dup_name_gender_by_key = dup_name_gender_by_key.merge(stats_best_name, how='left', on=[key, 'best_name'])
...
#                                                                   ascending=[True, False, True, False, False])
    ...
    if key == 'phone':
        condition_split = (dup_name_gender_by_key[key].str.len() == 11) | (dup_name_gender_by_key['customer_type'] != 'Ca nhan')

    elif key == 'email':
        condition_split = (dup_name_gender_by_key['customer_type'] != 'Ca nhan')

    com_dup_name_gender_by_key = dup_name_gender_by_key[condition_split].copy()
    per_dup_name_gender_by_key = dup_name_gender_by_key[~dup_name_gender_by_key[key].isin(com_dup_name_gender_by_key[key].unique())].copy()
    
    # unique company
    com_dup_name_gender_by_key = com_dup_name_gender_by_key.drop(columns=['active_date', 'last_active', 'priority', 'num_overall'])
    com_dup_name_gender_by_key['num_word'] = com_dup_name_gender_by_key['raw_name'].str.split(' ').str.len()
    com_dup_name_gender_by_key['num_char'] = com_dup_name_gender_by_key['raw_name'].str.len()
    com_dup_name_gender_by_key = com_dup_name_gender_by_key.sort_values(by=[key, 'num_word', 'num_char'], ascending=False)

    com_unique_name_gender_by_key = com_dup_name_gender_by_key.groupby(by=[key]).head(1)[[key, 'best_name', 'source_best_name']].drop_duplicates()
    com_unique_name_gender_by_key.columns = [key, 'unique_name', 'source_unique_name']

    # unique personal
    per_dup_name_gender_by_key['num_word'] = per_dup_name_gender_by_key['raw_name'].str.split(' ').str.len()
    ...
    per_dup_name_gender_by_key = per_dup_name_gender_by_key.groupby(by=[key]).head(1)[[key, 'best_name', 'source_best_name']].drop_duplicates()
    per_dup_name_gender_by_key.columns = [key, 'unique_name', 'source_unique_name']

    # concat unique key
    unique_name_gender_by_key = pd.concat([com_unique_name_gender_by_key, per_dup_name_gender_by_key], ignore_index=True)
    
    # merge key
    dup_name_gender_by_key = dup_name_gender_by_key.drop(columns=['active_date', 'last_active', 'priority', 'num_overall']).drop_duplicates()
    dup_name_gender_by_key = dup_name_gender_by_key.merge(unique_name_gender_by_key, on=[key], how='left')
    ...
    # OUTPUT
    name_gender_by_key = pd.concat([dup_name_gender_by_key, only_name_gender_by_key], ignore_index=True)
...
#                                    'best_name', 'source_best_name']
    ...
    # rename
    name_gender_by_key = name_gender_by_key.drop(columns=['name']).rename(columns={'raw_name': 'name'})
...
def PipelineBestName(date_str, key='phone'):
    ...
    # rename
    raw_names = raw_names.rename(columns={'raw_name': 'original_name'}).rename(columns={'name': 'raw_name'})
    ...
    pre_names.loc[pre_names['gender'] == pre_names['predict_gender'], 'best_gender'] = pre_names['gender']
    pre_names.loc[pre_names['best_gender'].isna(), 'best_gender'] = None
    pre_names = pre_names.drop(columns=['predict_gender'])
    
    # rename
    pre_names = pre_names.rename(columns={'raw_name': 'name'}).rename(columns={'original_name': 'raw_name'})
...
def DictNameGender():
    ...
    # rename columns
    data_name_cdp.columns = ['name', 'gender', 'source_name', 'enrich_name', 'enrich_gender', 'source_enrich_name']
    ...
    data_name_cdp_enrich = data_name_cdp[['enrich_name', 'enrich_gender', 'source_enrich_name']].copy()
    data_name_cdp_enrich.columns = ['name', 'gender', 'source_name']

    data_name_cdp = pd.concat([data_name_cdp_pre, data_name_cdp_enrich], ignore_index=True)
    data_name_cdp.loc[data_name_cdp['gender'].notna(), 'source_gender'] = data_name_cdp['source_name']
    ...
    data_name_cdp = pd.concat([data_name_cdp_gender, data_name_cdp_non_gender], ignore_index=True).drop_duplicates(subset=['name'], keep='first')
    ...
    data_name_cdp.loc[data_name_cdp['gender'].isna(), 'gender'] = data_name_cdp['predict_gender']
    ...
    data_name_cdp.loc[data_name_cdp['gender'].notna() & data_name_cdp['source_gender'].isna(), 'source_gender'] = 'Name2Gender'
    data_name_cdp = data_name_cdp.drop(columns=['name_en', 'name_len', 'predict_gender'])
    
    # Save
    data_name_cdp.to_parquet(f'{utils_path}/track_name_gender_latest.parquet', filesystem=hdfs, index=False)
