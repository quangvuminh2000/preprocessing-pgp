...
def FormatName(valid_email):
    ...
    name_email = valid_email[valid_email['username'].notna()][[
        'username']].drop_duplicates()

    # process format name
    # commercial_mask = ((name_email['customer_type'] != 'customer')
    #                    | (name_email['username'].str.split(' ').str.len() > 3))
    # com_name_email = name_email[commercial_mask][['username']]

    # per_name_email = name_email[~commercial_mask][['username']]
    # per_name_email = process_enrich(
    #     per_name_email,
    #     'username',
    #     n_cores=1
    # )
    ...
    # Fill Accent + gender
    map_name = process_enrich(
        name_email,
        'username',
        n_cores=1
    )
    map_name = map_name[['username', 'final_name']].rename(
        columns={'final_name': 'predict_name'}
    )

    map_name = process_predict_gender(
        map_name,
        name_col='predict_name',
        n_cores=1
    )

    valid_email = valid_email.merge(map_name, how='left', on=['username'])

    # ? Update Username, gender == predict name, gender
...
#     new_check_phones = check_phones[~check_phones['phone_raw'].isin(latest_check_phones['phone_raw'].unique())]
    ...
    if not check_emails.empty:
        # email_input = new_check_emails[['email_raw']].rename(
        #     columns={'email_raw': 'email'}).copy()
        email_output = process_extract_email_info(
            check_emails,
            'email_raw',
            n_cores=1
        )
        check_emails = check_emails.merge(
            email_output, how='left', on=['email_raw'])
        ...
        #     False)
        check_emails['export_date'] = today

        latest_check_emails = pd.concat(
            [latest_check_emails, check_emails], ignore_index=True)
