...
#             print(pct_vn)
    ...
    def CoreBestName(self, raw_names_n, name_col='name', key_col='phone'):
        ...
        names_df['unidecode_first_name'] = names_df['first_name'].apply(
            unidecode)
        ...
        # most unidecode_last_name
        stats_temp = names_df.groupby(by=['group_id', 'unidecode_last_name'])['group_id'].agg(count_freq='count').reset_index()
        stats_temp = stats_temp.sort_values(by=['group_id', 'count_freq'], ascending=[True, False])
        stats_temp = stats_temp.groupby(by=['group_id']).head(1)[['group_id', 'unidecode_last_name']]
        stats_temp.columns = ['group_id', 'mode_last_name']
        names_df = names_df.merge(stats_temp, how='left', on=['group_id'])
