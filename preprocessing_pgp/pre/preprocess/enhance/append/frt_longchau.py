...
def UnifyLongChau(
    ...
    profile_longchau.loc[profile_longchau['address'].str.len()
                         < 5, 'address'] = None
    profile_longchau['customer_type'] =\
        profile_longchau['customer_type'].replace({
            'Individual': 'Ca nhan',
            'Company': 'Cong ty',
            'Other': None
        })
...
#                                     filesystem=hdfs, columns = ['ShopCode', 'level1_norm', 'level2_norm', 'level3_norm']).drop_duplicates()
    ...
    profile_location_longchau['uid'] = profile_location_longchau['uid'].astype(str)
    ...
    profile_longchau['address'] = profile_longchau['address'].str.strip().replace(
        dict_trash)
    profile_longchau = profile_longchau.drop(columns=['city', 'street', 'ward', 'district'])
    profile_longchau = pd.merge(
        profile_longchau.set_index(['uid', 'address']),
        profile_location_longchau.set_index(['uid', 'address']),
        left_index=True, right_index=True,
        how='left', sort=False
    ).reset_index()
    ...
    profile_longchau.loc[profile_longchau['street'].notna(
    ), 'source_street'] = 'FSHOP from profile'

    # add info
    # profile_longchau['birthday'] = None
    columns = [
        'uid', 'phone', 'email', 'card',
        'name','gender', 'birthday', 'age', 'customer_type',
        'address', 'city', 'district', 'ward', 'street',
        'source_address', 'source_city', 'source_district', 'source_ward', 'source_street',
        'source',
    ]
