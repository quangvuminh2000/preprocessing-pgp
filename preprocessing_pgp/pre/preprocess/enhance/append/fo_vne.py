...
def UnifyFo(profile_vne: pd.DataFrame, n_cores: int = 1):
    ...
    # profile_fo.loc[
    #     profile_fo['customer_type'] == 'Ca nhan',
    #     'name'
...
def UpdateUnifyFo(now_str: str, n_cores: int = 1):
    # VARIABLES
    f_group = "fo_vne"
    yesterday_str = (
...
def UnifyLocationIpFo():
    ...
    most_ip_fo.to_parquet(
        f"{UTILS_PATH}/fo_location_most.parquet", index=False, filesystem=hdfs
    )
