...
def UnifyFo(date_str):
    ...
    profile_fo = profile_fo[[fo_cols]].copy()
    ...
    profile_fo = profile_fo.rename(
        columns={'phone': 'phone_raw', 'email': 'email_raw'})
    ...
    profile_fo.loc[profile_fo['age'] >
                   profile_fo['age'].quantile(0.99), 'age'] = np.nan
    # * 0/0/1945
    ...
    profile_fo.loc[profile_fo['age'].notna(), 'birthday'] =\
        (now_year - profile_fo['age']).astype(str)\
    ...
    profile_fo.loc[condition_name, 'name'] = profile_fo.loc[condition_name, 'name'].apply(
        clean_name_cdp)

    name_fo = profile_fo[profile_fo['name'].notna()][['name']
                                                     ].copy().drop_duplicates()
    # name_fo = preprocess_lib.ExtractCustomerType(name_fo)
    name_fo = process_extract_name_type(
        name_fo,
        name_col='name',
        n_cores=1,
        logging_info=True
...
#     profile_fo.loc[profile_fo['customer_type_detail'].isna(), 'customer_type_detail'] = None
    ...
    profile_fo.loc[
        profile_fo['name'] == profile_fo['username_email'],
        'name'
    ] = None
    profile_fo = profile_fo.drop(columns=['username_email'])

    # clean name
    condition_name = (profile_fo['customer_type'].isna())\
    ...
    profile_fo.loc[
        ...
        profile_fo.loc[condition_name, ]\
        .apply(lambda row: name_process.CleanName(row['name']), axis=1).tolist()
    profile_fo.loc[profile_fo['customer_type'].isna(
    ), 'name'] = profile_fo['clean_name']
    ...
    profile_fo.loc[
        profile_fo['name'].isin(skip_names),
        'name'
    ] = None
    ...
    # profile_fo_1.loc[
    #     (profile_fo_1['gender'].notna())
    #     & (profile_fo_1['gender'] != profile_fo_1['predict_gender']),
    #     'gender'
    # ] = None
