...
def UnifyFplay(profile_fplay, date_str='2022-07-01'):
    ...
    profile_fplay = profile_fplay.sort_values(by=['user_id', 'last_active', 'create_date'], ascending=False)
    profile_fplay = profile_fplay.drop_duplicates(subset=['user_id'], keep='first')
    profile_fplay = profile_fplay.drop(columns=['last_active', 'create_date'])
    ...
    profile_fplay.loc[condition_name, 'clean_name'] = profile_fplay.apply(lambda row: preprocess_lib.CleanName(row['name'], row['email']), axis=1)
...
def UpdateUnifyFplay(now_str):
    ...
    info_columns = ['user_id_fplay', 'phone', 'email', 'name', 'last_active', 'create_date']
