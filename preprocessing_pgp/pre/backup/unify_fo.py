...
def UnifyFo(profile_fo, date_str='2022-07-01'):
    ...
    # phone, email (valid)
    valid_phone = pd.read_parquet(f'/data/fpt/ftel/cads/dep_solution/sa/dev/utils/valid_phone.parquet/d={date_str}',
                                  filesystem=hdfs, columns=['phone_raw', 'phone', 'is_phone_valid'])
    valid_email = pd.read_parquet(f'/data/fpt/ftel/cads/dep_solution/sa/dev/utils/valid_email.parquet/d={date_str}',
                                  filesystem=hdfs, columns=['email_raw', 'email', 'is_email_valid'])

    # info
    profile_fo = profile_fo.rename(columns={'vne_id_fo': 'vne_id'})
    profile_fo = profile_fo.sort_values(by=['vne_id', 'last_active', 'create_date'], ascending=False)
    profile_fo = profile_fo.drop_duplicates(subset=['vne_id'], keep='first')
    profile_fo = profile_fo.drop(columns=['last_active', 'create_date'])
    ...
    profile_fo.loc[condition_name, 'clean_name'] = profile_fo.apply(lambda row: preprocess_lib.CleanName(row['name'], row['email']), axis=1)
...
def UpdateUnifyFo(now_str):
    ...
    info_columns = ['vne_id_fo', 'phone', 'email', 'name', 'gender', 'age', 'address', 'last_active', 'create_date']
    ...
    if difference_profile.empty:
        ...
        new_profile_unify = UnifyFo(difference_profile, date_str=yesterday_str)
    ...
    valid_phone = pd.read_parquet(f'/data/fpt/ftel/cads/dep_solution/sa/dev/utils/valid_phone.parquet/d={now_str}',
                                  filesystem=hdfs, columns=['phone_raw', 'phone', 'is_phone_valid'])
    valid_email = pd.read_parquet(f'/data/fpt/ftel/cads/dep_solution/sa/dev/utils/valid_email.parquet/d={now_str}',
                                  filesystem=hdfs, columns=['email_raw', 'email', 'is_email_valid'])
    ...
    # arrange columns
    columns = ['vne_id_fo', 'phone_raw', 'phone', 'is_phone_valid', 
               'email_raw', 'email', 'is_email_valid',
               'name', 'is_full_name', 'gender', 
               'birthday', 'customer_type', 'customer_type_detail',
               'address', 'unit_address', 'ward', 'district', 'city']
    ...
    profile_unify = profile_unify.drop_duplicates(subset=['vne_id_fo', 'phone_raw', 'email_raw'], keep='first')
