...
def BuildWordName(base_path):
    ...
    ext_data_name1["first_name"] = (
        ext_data_name1["full_name"].str.split(" ").str[-1]
    )
...
class NameProcess:
    ...
    def CleanName(self, raw_name):
        try:
            ...
            if len(process_name.split(" ")) > 4:
                process_name = re.sub(
                    regex_pronoun_remove, "", process_name
                ).strip()
            try:
            ...
            if last_name.strip() == "":
                ...
                while not check_end_case2:
                    for key_vi in self.last_name_list2:
                        ...
                        is_case21 = (
                            len(full_name) - full_name.rfind(key_vi)
                            == len(key_vi)
    ...
    def CoreBestName(self, raw_names_n, name_col="name", key_col="phone"):
        ...
        names_df["unidecode_first_name"] = names_df[
            "first_name"
        ].progress_apply(self.unidecode_with_na)
        names_df["group_id"] = (
        ...
        ] = names_df.loc[names_df["last_name"].notna(), "last_name"].apply(
            unidecode
        )
        ...
        map_names_n_df["num_word"] = (
            map_names_n_df["raw_name"].str.split(" ").str.len()
        )
        map_names_n_df["accented"] = map_names_n_df[
            "raw_name"
        ...
        names_n_df.loc[
            names_n_df["best_name"].isna(), "best_name"
        ] = names_n_df["raw_name"]
        ...
        for element_name in tqdm(["last_name", "middle_name", "first_name"]):
            # filter data detail
            map_element_name = (
                names_1_df[names_1_df[element_name].notna()][
            ...
            map_element_name["num_char"] = map_element_name[
                element_name
            ].str.len()
            ...
            map_element_name = map_element_name.groupby(by=["group_id"]).head(
                1
            )
        ...
        map_names_1_df.loc[
            map_names_1_df["best_name"].isna(), "best_name"
        ] = None
        # merge
        ...
        with mp.Pool(8) as pool:
            map_element_name[
                ["last_name", "middle_name", "first_name"]
            ] = pool.map(self.SplitName, map_element_name["name"])
        ...
        for element_name in tqdm(["last_name", "middle_name", "first_name"]):
            ...
            compare_names_df[f"similar_{element_name}"] = compare_names_df[
                f"raw_{element_name}"
            ].apply(unidecode) == compare_names_df[
                f"best_{element_name}"
            ].apply(
                unidecode
            )
        ...
        pre_names_df.loc[
            pre_names_df["best_name"].isna(), "best_name"
        ] = pre_names_df["raw_name"]
        pre_names_df.loc[
        ...
        pre_names_n.loc[
            pre_names_n["best_name"].isna(), "best_name"
        ] = pre_names_n["raw_name"]
        pre_names_n.loc[
            pre_names_n["similarity_score"].isna(), "similarity_score"
        ] = 1
        # Return
