...
__NAME_TYPE_PATH = os.path.join(
    os.path.dirname(__file__), "../../", "data", "name_type"
)

_NAME_TYPE_LV1_PATH = os.path.join(
    __NAME_TYPE_PATH, "customer_type_extra.parquet"
)

_NAME_TYPE_LV2_PATH = os.path.join(
    __NAME_TYPE_PATH, "customer_type_lv2.parquet"
)
...
LV1_NAME_TYPE_WITHOUT_ACCENT_REGEX = (
    LV1_NAME_TYPE.query("~is_accented")
    .groupby("ctype")["term"]
    .apply(list)
    .to_dict()
)
...
LV2_NAME_TYPE_REGEX = (
    LV2_NAME_TYPE.groupby("ctype")["term"].apply(list).to_dict()
)
